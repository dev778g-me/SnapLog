...
@Composable
fun View(screenshotFetchViewmodel: ScreenshotFetchViewmodel) {
    var screenshots by remember { mutableStateOf(emptyList<String>()) }
    val context = LocalContext.current
    val snapLogViewModel = SnapLogViewModel(context)
    LaunchedEffect(Unit) {
        screenshots = screenshotFetchViewmodel.getAllScreenshot(context)
    }
         Scaffold (
             topBar = {
                 TopAppBar(
             ...
             floatingActionButton = {
                 FloatingActionButton(onClick = {
                     snapLogViewModel.getDescriptionForAllImages(imagePathList = screenshots)
                 }) {
                     Text(text = "test")
                 }
             }
